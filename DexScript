...
Main = (function()
	...
	Main.FetchAPI = function()
		local api,rawAPI
        repeat task.wait() until Main.Elevated
		...
		if Main.LocalDepsUpToDate() then
			...
			rawAPI = game:HttpGet("http://setup.roblox.com/version-d0e8cfcd943d4ae2-API-Dump.json")
		...
		local function getMember(class,member)
	        ...
	        while currentClass do task.wait()
	...
	Main.FetchRMD = function()
		...
		repeat task.wait() until Main.Elevated
		if Main.LocalDepsUpToDate() then
				local localRMD = false
				if localRMD == true then 
					rawXML = localRMD
				else
					Main.DepsVersionData[1] = ""
				end
...
local classList = rootChildren[1].children
local enumList = rootChildren[2].children
...
for _, class in pairs(classList) do
    local className, data = "", {Properties = {}, Functions = {}}
            ...
            for _, member in pairs(child.children) do
                if member.attrs.class == "ReflectionMetadataMember" and member.children[1].tag == "Properties" then
                    ...
                    for _, prop in pairs(member.children[1].children) do
                        if prop.attrs then
                            local name = capitalize(prop.attrs.name)
                            memberData[name] = prop.children[1].text
                        end
                    end
                    if memberData.PropertyOrder then
                        propertyOrders[className] = propertyOrders[className] or {}
                        propertyOrders[className][memberData.Name] = tonumber(memberData.PropertyOrder)
                    end
                    classes[className].Properties[memberData.Name] = memberData
                end
            end
        elseif child.attrs.class == "ReflectionMetadataFunctions" then
            for _, member in pairs(child.children) do
                if member.attrs.class == "ReflectionMetadataMember" and member.children[1].tag == "Properties" then
                    ...
                    classes[className].Functions[memberData.Name] = memberData
                end
            end
        end
    end
    ...
    for _, child in pairs(enum.children) do
        ...
        elseif child.attrs.class == "ReflectionMetadataEnumItem" and child.children[1].tag == "Properties" then
            ...
            for _, prop in pairs(child.children[1].children) do
                local name = capitalize(prop.attrs.name)
                itemData[name] = prop.children[1].text
            end
            enums[enumName].Items[itemData.Name] = itemData
        end
    end
end
...
return {Classes = classes, Enums = enums, PropertyOrders = propertyOrders}
	...
	Main.Init = function()
		...
		API = Main.FetchAPI()
		Lib.FastWait()
		intro.SetProgress("Fetching RMD",0.2)
		RMD = Main.FetchRMD()
		Lib.FastWait()
